...
二、难点解决过程：
难点1：对于缺失值的填充，要首先把数据集中的“?”用NaN进行填充，然后再用中位数进行填充，经过测试用中位数填充的效果较好
难点2：采用神经网络进行训练时损失函数的选择，采用的是torch.nn.CrossEntropyLoss()损失函数，在用pytorch搭建分类网络时，不需要再在最后一个fc层再手动添加一个softmax层
难点3:最后怎么将预测结果输出
